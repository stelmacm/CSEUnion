...
myFormula <- (lossCost ~ as.factor(BusinessType) + Earnings + TotalIncurred + AmountOfInsurance) ## let's start with this random model...

library(statmod) ## the statmod library will allow us to use a tweedie distribution assumption
myModel <- glm(myFormula, data=Toronto,family=tweedie(var.power=1.5, link.power=0))
summary(myModel)

## We can now apply our model to predict the loss costs
Toronto$prediction <- predict(myModel,Toronto,type="response")
fixedExpensesPerPolicy <- prediction*0.1 #+ 15% of final premium + 12.5% of final premium
variableExpensesPerPolicy <- (150000 + 450000)
LAE <- fixedExpensesPerPolicy + variableExpensesPerPolicy 
