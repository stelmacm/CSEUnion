...
myFormula <- lossCost ~  Longitude + Latitude +  ## let's go with this Ok loss cost model
  ...
  I(CurrentClosingRatio^1) ## trying something with this ! 
...
LAE <- 0.1
profitLoading <- 0.06 ## $_$

marketPolicies$premium <- (marketPolicies$prediction*(1+LAE) + fixedExpensesPerPolicy)/(1-variableExpensesPerPolicy-profitLoading)

summary(marketPolicies$premium)

marketPolicies$premium[which(marketPolicies$CurrentClosingRatio > .8)] <- marketPolicies$premium[which(marketPolicies$CurrentClosingRatio > .8)] * 1.3

marketPolicies$premium[which(marketPolicies$CurrentClosingRatio < .2)] <- marketPolicies$premium[which(marketPolicies$CurrentClosingRatio < .2)] * .7

#lowones<- marketPolicies[which(marketPolicies$CurrentClosingRatio < .2),]
